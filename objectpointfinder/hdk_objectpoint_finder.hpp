...
namespace hdk {
	namespace objectpoint_finder {
		...
		bool find_objectpoint( gnd::path::path_net_area_and_speed_limited &path_net,
			...
			/* check destination */
			if ( path_net.index_objectpoint(objectpoint_name_destination) == -1 ) {
				...
				for ( j=i+1; j<path_net.n_objectpoints(); j++ ) {
					...
					if ( is_in_path( path_net, name_i, name_j, x, y, expansion )
						...
						{ // ---> compare
							...
							int ret_i;
							int ret_j;
							ret_i = path_net.find_path_dijkstra(&ws_i, name_i, objectpoint_name_destination);
							ret_j = path_net.find_path_dijkstra(&ws_j, name_j, objectpoint_name_destination);

							if ( ret_i < 0 || ret_j < 0 ) {
								return false;
							}
							if ( get_path_length(ws_i) > get_path_length(ws_j) ) {
								std::strncpy(objectpoint_name, name_i, 128);
								return true;
							} else {
								std::strncpy(objectpoint_name, name_j, 128);
								return true;
							}
						} // <--- compare
