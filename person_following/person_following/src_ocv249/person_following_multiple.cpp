...
int main(int argc, char **argv)
{
	...
	for (int jj = 0; jj < SCAN_STEP_NUM; jj++)
	{
		...
		cvCircle(baseImage, cvPoint(cvRound(ThresS * Scale * sin(angle)) + OffsetX, cvRound(ThresS * Scale * cos(angle)) + OffsetY), 1, CV_RGB(64, 64, 64), -1, 8); // 近距離のしきい値の円孤
		cvCircle(baseImage, cvPoint(cvRound(ThresM * Scale * sin(angle)) + OffsetX, cvRound(ThresM * Scale * cos(angle)) + OffsetY), 1, CV_RGB(32, 32, 32), -1, 8); // 中距離のしきい値の円弧
		cvCircle(baseImage, cvPoint(cvRound(ThresL * Scale * sin(angle)) + OffsetX, cvRound(ThresL * Scale * cos(angle)) + OffsetY), 1, CV_RGB(64, 64, 64), -1, 8); // 遠距離のしきい値の円弧
	}
			...
			for(const auto &itr : handlerList){
				for (int i = 0; i < itr.urgDistance.size(); i++)
				{
					...
					{																						   //1m以内で正規化反射強度が120以上なら追跡初期化
						initValue->data.fl[0] = cvRound((itr.urgDistance[i] * Scale * itr.sinVal[i]) + OffsetX - 20); // サンプルの初期化パラメータのうち初期値をマウスクリック座標に設定する
						initValue->data.fl[1] = cvRound((itr.urgDistance[i] * Scale * itr.cosVal[i]) + OffsetY);
						myConDensInitSampleSet(ConDens, initValue, initMean, initDeviation); // 追跡器構造体Condensの各サンプルの初期化
						myConDensUpdateSample(ConDens);										 // 追跡器構造体Condensのサンプルの更新
						myConDensUpdateByTime(ConDens);										 // 状態期待値も計算しておく
						isTracked = 1;
