...
double URG_Distance_I[SCAN_STEP_NUM], URG_Intensity_I[SCAN_STEP_NUM], angle[SCAN_STEP_NUM], angle_I[SCAN_STEP_NUM], a_1[SCAN_STEP_NUM], a_2[SCAN_STEP_NUM], b_1[SCAN_STEP_NUM], b_2[SCAN_STEP_NUM], c[SCAN_STEP_NUM]; //反射強度正規化用
...
int main(int argc, char **argv)
{
	...
	double defOffsetPoseX = 0.0;
	double defOffsetPoseY = -0.6;
	double defOffsetPoseT = 0.0;
	...
	int INTENS_S = 12000;//120000;				 //正規化反射強度の下限
	...
	ros::Subscriber subLaserScan =  nodeHandle.subscribe(defTopicNameScan.c_str(), 100,
	...
	while (ros::ok())
	{
		...
		//分散値計算
		for (int lp = 0; lp < 3; lp++)
		{																			 //有効サンプルの分散を計算(lp-次元　0:x,1:y,2:角度)
		...
		for (int i = 0; i < SCAN_STEP_NUM; i++)
		{
			...
			{ //対象損失時:k=0
				...
				{ //射程枠内に
					if (URG_Intensity_I[i] > INTENS_S && URG_Intensity_I[i] < INTENS_L)
					{ //正規化反射強度が閾値以上なら初期値変更
						initValue->data.fl[0] = (float)x[i];
						initValue->data.fl[1] = (float)y[i];								 // サンプルの初期化パラメータのうち初期値をマウスクリック座標に設定する
						myConDensInitSampleSet(ConDens, initValue, initMean, initDeviation); // 追跡器構造体Condensの各サンプルの初期化
						myConDensUpdateSample(ConDens);										 // 追跡器構造体Condensのサンプルの更新
						myConDensUpdateByTime(ConDens);										 // 状態期待値も計算しておく
						k = 1;
		...
		// For Follow Tracking
		double rotX = diffX * cos(defOffsetPoseT) - diffY * sin(defOffsetPoseT);
		double rotY = diffX * sin(defOffsetPoseT) + diffY * cos(defOffsetPoseT);
		...
		msgPosePersonFollowing.theta = defOffsetPoseT + usrAngl;
