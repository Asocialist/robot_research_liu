...
class ScanMessageHandler
{
	...
	void CalcDrawPosition()
	{
		...
		for (int i = 0; i < arrayMinimal; i++)
		{
			...
			{
				drawPosition[i].x = cvRound(urgDistance[i] * Scale * sinVal[i]) + windowSize.width * 0.5 - offsetY * Scale;
				drawPosition[i].y = cvRound(urgDistance[i] * Scale * cosVal[i]) + windowSize.height * 0.5 - offsetX * Scale;
			}
		}
	}
...
int main(int argc, char **argv)
{
	...
	{
		cv::Mat ___t(cv::Size(sensorVisibleRange[0]->width,sensorVisibleRange[0]->height),CV_8UC1);
	...
	cv::circle(baseImage, cv::Point(cv::round(OffsetX_), cv::round(OffsetY_)), 4, CV_RGB(64, 64, 64), -1); // センサの位置
	for (int jj = 0; jj < SCAN_STEP_NUM; jj++)
	{
		double angle = (jj * 0.25 + Rotate) * (M_PI / 180);
		cv::circle(baseImage, cv::Point(cv::round(ThresS * Scale * sin(angle)) + OffsetX_, cv::round(ThresS * Scale * cos(angle)) + OffsetY_), 1, CV_RGB(64, 64, 64), -1); // 近距離のしきい値の円孤
		cv::circle(baseImage, cv::Point(cv::round(ThresM * Scale * sin(angle)) + OffsetX_, cv::round(ThresM * Scale * cos(angle)) + OffsetY_), 1, CV_RGB(32, 32, 32), -1); // 中距離のしきい値の円弧
		cv::circle(baseImage, cv::Point(cv::round(ThresL * Scale * sin(angle)) + OffsetX_, cv::round(ThresL * Scale * cos(angle)) + OffsetY_), 1, CV_RGB(64, 64, 64), -1); // 遠距離のしきい値の円弧
	}
				...
				*/
				if( loopNumber == 1 && (i == 0 || i == (itr.drawPosition.size() - 1)) )
				{
					...
					if(i == (itr.drawPosition.size() - 1) && itr.offsetY > 0) //右側LiDAR&&最後の点
					{
						...
						cv::fillConvexPoly(maskImage, pPolyPoints2, 4, CvScalar(255), 8, 0);
						//printf("scan[%d]=(%lf,%lf)", i, itr.drawPosition[i].x, itr.drawPosition[i].y);
						//printf("kou2=%lf\n", kouten_x2);
					}
					//printf("wsx=%d, wsy=%d \n", windowSize.x, windowSize.y);
			...
			for(const auto &itr : handlerList){
				...
				for (int i = 0; i < itr.urgDistance.size(); i++)
				{
					...
					{																						   //1m以内で正規化反射強度が120以上なら追跡初期化
						...
						usrU = cv::round((itr.urgDistance[i] * Scale * itr.sinVal[i]) + OffsetX_ - 20); // サンプルの初期化パラメータのうち初期値をマウスクリック座標に設定する
						usrV = cv::round((itr.urgDistance[i] * Scale * itr.cosVal[i]) + OffsetY_);
